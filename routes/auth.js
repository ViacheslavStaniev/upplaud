...
const { registerUserWithAccount, getUserInfo } = require("./users");
...
router.get("/me", verifyAuth, async (req, res) => {
  try {
    let user = await getUserInfo(req.user.id);
...
router.post("/social_login", [check("token", "Invalid token. Please try again.").not().isEmpty()], async (req, res) => {
  ...
  try {
    ...
    if (user) {
      ...
      user = await registerUserWithAccount(given_name, family_name, email, randomString(12), timezone);
    }
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      const user = await registerUserWithAccount(firstname, lastname, email, password, timezone);
