...
router.get("/init-auto-posting", async (req, res) => {
  try {
    ...
    const activePostings = await SocialPosting.find({ isActive: true, nextPostDate: { $lte: next10Minutes } })
    ...
    for (let i = 0; i < activePostings.length; i++) {
      ...
      if (type === SOCIAL_TYPE.FACEBOOK) {
        try {
          ...
          postingResults.push({ type, subType, error: false, msg: "Posted on Facebook" });
        } catch (error) {
          console.log(error);
          postingResults.push({ type, subType, error: true, msg: error?.message || "Error posting on Facebook" });
        }
        ...
        try {
          ...
          postingResults.push({ type, subType, error: false, msg: "Posted on Linkedin" });
        } catch (error) {
          // await saveUserAccessTokens(user, { type, subType, isConnected: false, expiresInSeconds: 0 });
          console.log(error);
          postingResults.push({ type, subType, error: true, msg: error?.message || "Error posting on Linkedin" });
        }
      }
    }
